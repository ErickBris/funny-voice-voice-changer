...
public class MaterialDialog extends DialogBase implements View.OnClickListener, AdapterView.OnItemClickListener {
	...
	protected MaterialDialog(Builder builder) {
		...
		if (mBuilder.mIndeterminateProgress || mBuilder.mProgress > -2) {
			mBuilder.customView = inflater.inflate(mBuilder.mIndeterminateProgress ? R.layout.md_progress_dialog_indeterminate : R.layout.md_progress_dialog,
	...
	/**
	 * To account for scrolling content and overscroll glows, the frame
	 * padding/margins sometimes must be set on inner views. This is dependent
	 * on the visibility of the title bar and action buttons. This method
	 * determines where the padding or margins are needed and applies them.
	 */
	...
	/**
	 * Invalidates visibility of views for the presence of a custom view or list
	 * content
	 */
	 ...
	 * is removed as it makes the button bar look off balanced with different
	 * amounts of padding above and below the divider.
	 */
	private void setDividerVisibility(boolean topVisible, boolean bottomVisible) {
		...
		if (bottomVisible) {
			 ...
			 * content to scroll, which is bad experience itself but it also
			 * causes a vibrating window as this will keep getting
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		if (listType != null) {
			...
			if (listType == ListType.MULTI) {
				...
				boolean isChecked = !((CheckBox) view.findViewById(R.id.control)).isChecked(); // Inverted
																								// because
																								// the
																								// view's
																								// click
	...
	/**
	 * Measures the action button's and their text to decide whether or not the
	 * button should be stacked.
	 */
	...
	/**
	 * Invalidates the positive/neutral/negative action buttons. Decides whether
	 * they should be visible and sets their properties (such as height, text
	 * color, etc.).
	 */
	private boolean invalidateActions() {
		...
		if (mBuilder.negativeText != null) {
			...
			if (!isStacked) {
				RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, (int) getContext().getResources().getDimension(
	...
	public final void onClick(View v) {
		...
		else if (tag.equalsIgnoreCase(NEUTRAL)) {
			if (mBuilder.callback != null)
	...
	public static class Builder {
		 ...
		 * {@link #typeface(String, String)} and
		 * {@link #typeface(Typeface, Typeface)}.
		 * 
		 ...
		 * 
		 * @param medium
		 *            The name of font in assets/fonts, minus the extension
		 *            (null uses device default). E.g.
		 *            [your-project]/app/main/assets/fonts/[medium].ttf
		 * @param regular
		 *            The name of font in assets/fonts, minus the extension
		 *            (null uses device default). E.g.
		 *            [your-project]/app/main/assets/fonts/[regular].ttf
		 * @return The Builder instance so you can chain calls to it.
		 */
	...
	/**
	 * This will not return buttons that are actually in the layout itself,
	 * since the layout doesn't contain buttons. This is only implemented to
	 * avoid crashing issues on Huawei devices. Huawei's stock OS requires this
	 * method in order to detect visible buttons.
	 * 
	 * @deprecated Use
	 *             getActionButton(com.afollestad.materialdialogs.DialogAction)}
	 *             instead.
	 */
	@Deprecated
	@Override
	...
	/**
	 * Retrieves the frame view containing the title and icon. You can manually
	 * change visibility and retrieve children.
	 */
	...
	public final void setContent(CharSequence content) {
		...
		invalidateCustomViewAssociations(); // invalidates padding in content
											// area scroll (if needed)
	}
	...
	public final void setItems(CharSequence[] items) {
		...
		if (mBuilder.adapter instanceof MaterialDialogAdapter) {
			mBuilder.adapter = new MaterialDialogAdapter(mBuilder.context, ListType.getLayoutForType(listType), R.id.title, items);
		}
		else {
	...
	/**
	 * Use this to customize any list-specific logic for this dialog
	 * (OnItemClickListener, OnLongItemClickListener, etc.)
	 * 
	 * @return The ListView instance used by this dialog, or null if not using a
	 *         list.
	 */
	@Nullable
